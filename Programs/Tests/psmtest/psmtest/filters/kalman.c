...
void InitializeRhoKalman( kalman_filter_t * k, floating_t v, floating_t ls, index_t minv, index_t maxv, kalman_uncertainty_c uncertainty )
{
    k->lifespan    = ls;
...
void ResetRhoKalman( kalman_filter_t * k, floating_t v )
{
    k->K[0]        = 0;
    k->K[1]        = 0;
    k->P[0][0]     = 0;
...
void PredictRhoKalman( kalman_filter_t * k, floating_t rate_new )
{
...
void UpdateRhoKalman( kalman_filter_t * k, floating_t value_new )
{
...
floating_t StepRhoKalman( kalman_filter_t * k, floating_t value_new, floating_t rate_new )
{
...
bool IsRhoKalmanExpired( kalman_filter_t * k )
{
    return ((TIMESTAMP() - k->timestamp) > k->lifespan);
}

inline floating_t ScoreRhoKalman( kalman_filter_t * k )
{
...
void PunishRhoKalman( kalman_filter_t * k )
{
