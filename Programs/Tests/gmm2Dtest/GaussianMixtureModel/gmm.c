...
{
    cluster->mean_in.a = input->a;
    cluster->mean_in.b = input->b;
    cluster->mean_out.a = output->a;
    cluster->mean_out.b = output->b;
    
    memset( &cluster->covariance_in_out, 0, sizeof(mat2x2) );
    cluster->covariance_in.a = variance; cluster->covariance_in.d = variance;
    cluster->covariance_in.b = 0.; cluster->covariance_in.c = 0.;
    cluster->score = 1.;
    
    GetMat2x2LLT( &cluster->covariance_in, &cluster->llt_in );
...
void UpdateGaussianMixtureCluster( gaussian_mixture_cluster_t * cluster, vec2 * input, vec2 * output )
{
    ...
    vec2SubVec2( input, &cluster->mean_in, &mean_in_delta );
    scalarMulVec2( weight, &mean_in_delta, &cluster->mean_in );
    vec2SubVec2( output, &cluster->mean_out, &mean_out_delta );
    scalarMulVec2( weight, &mean_out_delta, &cluster->mean_out );
    ...
    mat2x2SubMat2x2( &unweighted_in_covariance_factor, &cluster->covariance_in, &covariance_delta_factor );
    scalarMulMat2x2( weight, &covariance_delta_factor, &covariance_delta_factor );
    mat2x2AddMat2x2( &cluster->covariance_in, &covariance_delta_factor, &cluster->covariance_in );
    ...
    mat2x2SubMat2x2( &unweighted_in_out_covariance_factor, &cluster->covariance_in, &covariance_delta_factor );
    scalarMulMat2x2( weight, &covariance_delta_factor, &covariance_delta_factor );
    mat2x2AddMat2x2( &cluster->covariance_in_out, &covariance_delta_factor, &cluster->covariance_in_out );
    
    GMMFunctions.Cluster.UpdateNormal( cluster );
    
    GetMat2x2LLT( &cluster->covariance_in, &cluster->llt_in );
    getMat2x2Inverse( &cluster->covariance_in, &cluster->inv_covariance_in );
}
void GetScoreOfGaussianMixtureCluster( gaussian_mixture_cluster_t * cluster, vec2 * input)
{
    vec2 input_delta;
    vec2SubVec2(&cluster->mean_in, input, &input_delta);
...
void ContributeToOutputOfGaussianMixtureCluster( gaussian_mixture_cluster_t * cluster, vec2 * input, vec2 * output )
{
    ...
    mat2x2DotVec2( &cluster->covariance_in_out, &inv_covariance_delta, &input_covariance );
    vec2AddVec2( &cluster->mean_out, &input_covariance, &pre_condition);
