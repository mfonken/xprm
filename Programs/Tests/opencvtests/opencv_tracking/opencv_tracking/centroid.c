    ...
    for( i = 0; i < *num_blobs; i++ )
    {
        if( ( y - c->centroids.blobs[i].y_last ) <= MAX_GAP )    // Ensure blob hasn't expired
        {
                ...
                ( ( x - n_c - MAX_GAP ) <= ( x_l + n_l ) ) ) // and of upper bound of blob and lower (with gap tolerance) of new
            {
                ...
                {
                    cma( c->centroids.blobs[i].X, &c->centroids.blobs[id].X, 3. );
                    cma( c->centroids.blobs[i].Y, &c->centroids.blobs[id].Y, 3. );
...
{
    uint16_t gap = 0xffff;
    uint8_t temp_id = 0;   // Global variables
    double a_x_last = 0, num_adj = 0;                  // Global last X and Y averages
    uint16_t x;
    for( x = 0; x < c->CENTROIDS_WIDTH; x += skip )            // Traverse all columns
    {
        ...
        else if( gap != 0xffff )                          // Otherwise, if gap counter is counting (i.e. there was a recent pixel
        {
            ...
            {                                               // Include last pixel into a blob
                ...
                cma( (double)line_number, &c->centroids.blobs[temp_id].Y, ++c->centroids.blobs[temp_id].height ); // Cumulate the new row into the blob's Y average
...
//                c->centroids.blobs[temp_id].height++;            // height
                ...
                gap = 0xffff;                           // Reset the gap to NULL_
            }
        }
    }
}
...
{
    memset(c, 0, sizeof(centroids_test_t));
