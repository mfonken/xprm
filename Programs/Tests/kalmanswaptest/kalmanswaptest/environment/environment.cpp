...
Environment::Environment( TestInterface * test, int rate )
{
    printf("Initializing Test Environment.\n");
    if (pthread_mutex_init(&lock, NULL) != 0) printf("\n mutex init failed\n");
    if(events.add( &lock, test, rate ))
        test->init();
    status = INITIALIZED;
}
...
Event::Event( pthread_mutex_t * mutex, TestInterface * test, int rate )
{
    this->id = 0;
    this->mutex = mutex;
    this->test = test;
...
int EventList::add( pthread_mutex_t* mutex, TestInterface* test, int rate )
{
