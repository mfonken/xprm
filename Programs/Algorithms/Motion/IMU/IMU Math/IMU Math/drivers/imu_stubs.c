...
void Read_SERCOM_IMU_Packet( int filestream, LSM9DS1_t * imu )
{
    ...
    {
        bytes_read = 0;
        while(bytes_read <= 0) bytes_read = Read_SERCOM_Bytes(filestream, buffer, (size_t)BUFFER_LENGTH);
        Read_SERCOM_Bytes(filestream, buffer, (size_t)BUFFER_LENGTH);
