...
void TauDraw::drawDensitiesOnFrame(Mat M)
{
    ...
    int Qv[] = { tau->rho.Q[0], tau->rho.Q[1], tau->rho.Q[2], tau->rho.Q[3] };
//    qsort(Qv, 4, sizeof(int), compare);
#define inset_ 50
#define fontsize_ 2
#define fontcolor_ Vec3b(100,100,0)
    putText(frame, to_string(Qv[0]), Point(inset_, inset_), FONT_HERSHEY_PLAIN, fontsize_, fontcolor_, 4);
    putText(frame, to_string(Qv[1]), Point(w-inset_, inset_), FONT_HERSHEY_PLAIN, fontsize_, fontcolor_, 4);
    putText(frame, to_string(Qv[2]), Point(inset_, h-inset_), FONT_HERSHEY_PLAIN, fontsize_,fontcolor_, 4);
    putText(frame, to_string(Qv[3]), Point(w-inset_, h-inset_), FONT_HERSHEY_PLAIN, fontsize_, fontcolor_, 4);
    
    putText(frame, "Thresh: " + to_string(tau->utility->thresh), Point(0, 18), FONT_HERSHEY_PLAIN, 1, Vec3b(255,0,255), 2);
    
    putText(frame, "X (" + to_string(tau->rho.Cx) + ", " + to_string(tau->rho.Cy) + ")", Point(tau->rho.Cx, tau->rho.Cy), FONT_HERSHEY_PLAIN, 2, Vec3b(0,255,255), 4);
