 ...
 * b - length of vec d'
 */
static void chi( kinetic_t * k )
{
    double m, n;
    m = ZDIV( k->f_l, cos(k->sigmaA) );
    n = ZDIV( m, k->d__l );
    k->chi = RASIN( n * sin(k->sigmaR) );
}
 ...
 * b - d vec real length
 */
static void r_l( kinetic_t * k )
{
    double m,n;
    m = sin( k->gam );
    n = ZDIV( m , sin( k->sigmaR ) );
    k->r_l = n * k->d_l;
}
